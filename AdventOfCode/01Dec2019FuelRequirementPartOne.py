...
def calculateFuelForMass(moduleMasses):
...
def getFuelRequirementForMass(moduleMass):
    return math.floor(moduleMass/3) - 2


assert (2 == calculateFuelForMass([12]))
assert (2 == calculateFuelForMass([14]))
assert (654 == calculateFuelForMass([1969]))
assert (33583 == calculateFuelForMass([100756]))
assert (4 == calculateFuelForMass([14, 12]))

print(calculateFuelForMass([138486, 133535, 66101, 98143, 56639, 120814, 142212, 92654, 100061, 104095, 55169, 94082, 76014, 81109, 106237, 111930, 138463, 145843, 142133, 71154, 112809, 136465, 142342, 68794, 131804, 146345, 107935, 98577, 127456, 89612, 95710, 149792, 136982, 92773, 92303, 114637, 107447, 111815, 149603, 106822, 78811, 114120, 148773, 90259, 101612, 82220, 139301, 91121,
